...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_Model->Initialize(m_D3D->GetDevice(), m_D3D->GetDeviceContext(), "../Engine/data/stone01.tga", "../Engine/data/cube.txt");
	if(!result)
	{
	...
	result = m_DebugWindow->Initialize(m_D3D->GetDevice(), screenWidth, screenHeight, 150, 150);
	if(!result)
	{
...
bool GraphicsClass::Render()
{
	...
	result = m_DebugWindow->Render(m_D3D->GetDeviceContext(), 0, 0);
	if(!result)
	{
		return false;
	}
